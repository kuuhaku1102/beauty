...
def clean_text(s): return re.sub(r"\s+", " ", s).strip() if s else ""
def to_int(s):
...
def parse_card(card, base_url=None):
    ...
    for li in card.select("ul li a.small-list__item"):
        ...
        menus.append({"title": title, "price_jpy": price_jpy, "price_raw": price_text, "url": menu_url, "pickup_flag": pickup, "category_raw": cat})
    hours = parse_hours(card.select_one("table.table"))
    return {"rank": rank, "name": name, "clinic_url": clinic_url, "rating": rating, "reviews": reviews, "snippet": snippet, "snippet_author": snippet_author, "images": images, "features": features, "access": access_text, "hours": hours, "menus": menus}
...
def parse_page(html, page_url):
    ...
    if not cards:
        ...
        cards.append({"rank": None, "name": h1 or title, "clinic_url": page_url, "rating": None, "reviews": None, "snippet": "", "snippet_author": "", "images": [], "features": [], "access": "", "hours": {}, "menus": []})
    return cards
...
def append_rows(ws, rows):
    ...
    if values: ws.append_rows(values, value_input_option="RAW")
...
def write_to_sheet(rows):
    ...
    except Exception: ws = sh.add_worksheet(title=worksheet_name, rows=2000, cols=20)
    header = ["timestamp_utc","source","url","title","h1","status","notes"]
    ensure_header(ws, header); append_rows(ws, rows)
    print(f"[Sheets] Appended {len(rows)} rows to '{worksheet_name}'.")

def write_settings_sheet():
    """
    settings シートに機微情報を漏らさず記録：
      - TARGET_URLS, GSHEET_KEY, GSHEET_WORKSHEET
      - GSHEET_JSON_B64 は『長さ＆マスク』のみ
    """
    ...
    if not (json_b64 and sheet_key):
        print("[Settings] Skipped (env not set)."); return
    gc = get_gspread_client_from_b64(json_b64)
    sh = gc.open_by_key(sheet_key)
    try: ws = sh.worksheet(settings_sheet_name)
    except Exception: ws = sh.add_worksheet(title=settings_sheet_name, rows=100, cols=4)
    ...
    values = [
        {"key":"TARGET_URLS",       "value": os.getenv("TARGET_URLS",""),  "note":"スクレイピング対象URL(カンマ区切り)", "updated_utc": now_utc_iso()},
        {"key":"GSHEET_KEY",        "value": sheet_key,                     "note":"スプレッドシートID",                 "updated_utc": now_utc_iso()},
        {"key":"GSHEET_WORKSHEET",  "value": worksheet_name,                "note":"結果出力シート名",                   "updated_utc": now_utc_iso()},
        {"key":"GSHEET_JSON_B64",   "value": masked_summary(os.getenv("GSHEET_JSON_B64","")), "note":"Secretsに格納。値は保存しない。", "updated_utc": now_utc_iso()},
    ]
...
def main():
    ...
    if not target_urls: raise SystemExit("TARGET_URLS is empty")
    ...
    for url in urls:
        ...
        for c in cards:
            ...
            all_rows.append({
                "timestamp_utc": ts, "source": "requests",
                "url": c.get("clinic_url", url),
                "title": c.get("name",""),
                "h1": c.get("name",""),
                "status": "ok", "notes": ", ".join(notes)
            })
