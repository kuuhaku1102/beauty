...
TIMEOUT = 30
RETRY = 3
SLEEP_BETWEEN = 2  # polite delay between requests
...
def build_target_urls_auto():
    ...
    if not start_id_str.isdigit() or not end_id_str.isdigit():
        raise SystemExit("START_ID and END_ID must be numeric")
...
def extract_menus_from_scope(scope, base_url=None):
    ...
    for a in scope.select("a.small-list__item"):
        ...
        menus.append({
            ...
            "category_raw": clean_text(a.select_one(".treatment-category") and a.select_one(".treatment-category").get_text()),
            "menu_img": "",
        })
    return menus
...
def parse_card(card, base_url=None):
    ...
    access_text = clean_text(card.select_one(".card__detail") and card.select_one(".card__detail").get_text()) or \
        clean_text(card.select_one(".card__access-text") and card.select_one(".card__access-text").get_text())
...
def write_settings_sheet():
    ...
    now = now_utc_iso()
    values = [
        {"key": "START_ID", "value": os.getenv("START_ID", ""), "note": "自動探索の開始ID", "updated_utc": now},
        {"key": "END_ID", "value": os.getenv("END_ID", ""), "note": "自動探索の最終ID", "updated_utc": now},
